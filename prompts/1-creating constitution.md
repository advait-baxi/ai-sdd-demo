# Create Constitution

## Prompt 1

```
We are writing AgentClinic, a place for AI agents to get relief from their humans. Look in the src/README.md for input from stakeholders.

Let's create a "constitution" in a specs directory:

    mission.md
    tech-stack.md
    roadmap.md for high-level implementation order, in very small phases of work.

Important: You must use your AskUserQuestion tool, grouped on these 3, before writing to disk.
```

## Prompt 2

While agents asks to select tech stack, provide following custom answer as a prompt.

```
Use server-side TypeScript and recommend a framework.
```
